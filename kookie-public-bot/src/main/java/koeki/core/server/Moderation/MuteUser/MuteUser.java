...
public class MuteUser extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(@Nonnull GuildMessageReceivedEvent event) {
        ...
        if(content.startsWith("k@mute")){
            if(event.getMember().hasPermission(Permission.KICK_MEMBERS) || event.getMember().hasPermission(Permission.ADMINISTRATOR) && !event.getMember().getUser().isBot()){
                ...
                try{
                    if(roleList.isEmpty()){
                        ...
                        String StringTimer = content.substring(30);
                        int IndexM = 0;
                        if(StringTimer.contains("m")){
                            IndexM = StringTimer.indexOf("m");
                        }
                        Long timer = Long.parseLong(StringTimer.substring(0, IndexM));
